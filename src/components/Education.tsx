  ...
  return (
     <section id="educacao" className="p-4 w-full scroll-mt-24">
      <div className="bg-[rgba(100,100,100,0.2)] rounded-2xl shadow-lg p-6 border border-zinc-700">
        <h2 className="text-3xl font-bold text-white mb-2">Educação</h2>
        <div className="h-2 w-80 bg-gradient-to-r from-cyan-400 to-white rounded-full mb-6" />
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 text-white">
          {cursos.map((curso, index) => (
            <div
              key={index}
              className="relative bg-white/10 border border-cyan-500 rounded-xl p-4 hover:scale-105 transition overflow-hidden"
            >
              ...
              {/* Linha azul inferior */}
              <div className="absolute bottom-0 left-0 w-[99%] ml-1 h-[4px] bg-cyan-200 rounded-full" />
            </div>
          ))}
        </div>
      </div>
