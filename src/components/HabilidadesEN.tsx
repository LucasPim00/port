      ...
      <div className="bg-[rgba(100,100,100,0.2)] rounded-2xl shadow-lg p-6 border border-zinc-700">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 text-white">
          {skills.map((skill) => (
            <div
              key={skill.name}
              className="flex flex-col items-center justify-between gap-2 bg-white/10 border border-red-500 rounded-xl p-4 hover:scale-105 transition relative"
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 w-[98%] ml-1 h-[4px] bg-red-600 rounded-full" />
            </div>
          ))}
        </div>
      </div>
