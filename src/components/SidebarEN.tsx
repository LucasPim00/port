...
const menuItems = [
  { key: 'About Me', icon: <FaUser />, href: '#sobre' },
  { key: 'Projects Front-end', icon: <FaGamepad />, href: '#projetos' },
  { key: 'Projects Back-end', icon: <FaGaugeSimple />, href: '#projetosB' },
  { key: 'Projects FullStack', icon: <FaGalacticRepublic />, href: '#projetosF' },
  { key: 'Skills', icon: <FaTools />, href: '#habilidades' },
  { key: 'Education', icon: <FaGraduationCap />, href: '#educacao' },
  { key: 'Experiences', icon: <FaBriefcase />, href: '#experience' },
];
...
export default function Sidebar({ modoColapsado, animando }: SidebarProps) {
  return (
    ...
    >
      {!modoColapsado && (
        <>
          <div className="w-52 h-52 rounded-full overflow-hidden border-4 border-red-500 mb-8">
            <Image
              src="/avatar.jpg"
              alt="Avatar"
              width={96}
              height={96}
              className="w-44 h-44 mb-4 avatar-neon-red mt-4 ml-3"
            />
          </div>

          <h1 className="px-8 text-lg hover:text-yellow-500 transition-colors cursor-crosshair text-center text-white relative -mt-4 pb-1 after:content-[''] after:absolute after:-bottom-1 after:left-1/2 after:-translate-x-1/2 after:w-1/2 after:border-b-4 after:border-red-400 after:rounded-full after:shadow-[0_0_10px_red]">
            🤖 LUCAS PIM 🤖
            <span className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-28 h-[2px] bg-red-500 shadow-[0_0_6px_2px_rgba(255,0,0,0.8)] rounded-full"></span>
          </h1>
        </>
      )}
      ...
      {!modoColapsado && (
        <div className="mt-auto pl-1 px-6 pt-2">
          <div className="mb-3 text-m text-gray-400 hover:text-orange-400 transition cursor-default">
